    ...
    <Card className="flex flex-col overflow-hidden hover:shadow-2xl transition-all border-primary/20 hover:border-primary/50 bg-card/50 backdrop-blur group">
      ...
      <CardHeader className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          {post.game && (
            <Badge variant="outline" className="border-primary/30 font-medium text-xs">
      ...
      <CardContent className="space-y-4">
        <p className="text-muted-foreground line-clamp-3 leading-relaxed">{post.title}</p>

        <div className="flex flex-wrap gap-2">
          {post.tags.length > 0 &&
            post.tags.map((tag) => (
              <Link key={tag.id} to={`/tag/${tag.slug}`}>
                <Badge variant="outline" className="hover:bg-primary/10 hover:border-primary/50 transition-all text-xs">
                  #{tag.title}
                </Badge>
              </Link>
            ))}
      ...
      <CardFooter className="flex mt-auto items-center justify-between border-t border-primary/10 pt-4">
        ...
        >
          <Avatar className="h-8 w-8 border-2 border-primary/20">
            <AvatarImage src={post.author?.avatar?.url || "/placeholder.svg"} alt={post.author.name} />
