...
const commentSchema = object({
  ...
  replyToCommentId: generateStringSchema("replyToId", undefined, undefined, false).nullable(),
})
...
const MutateCommentDialog = (props: MutateCommentDialogProps) => {
  ...
  const { control, handleSubmit } = useForm<FormSchema>({
    ...
    defaultValues: commentToEdit ? { comment: commentToEdit.content, replyToCommentId: replyToComment?.id } : undefined,
  })
  ...
  const onSubmit = (data: Required<FormSchema>) => {
    if (isEditMode) updateComment.mutate({ commentId: commentToEdit.id, postId, ...data })
    else createComment.mutate({ postId, ...data })
  }

  const disabled = loading || (isEditMode ? updateComment.isPending : createComment.isPending)
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isEditMode ? t("comment.editComment") : t("comment.addNewComment")}</DialogTitle>
          <DialogDescription>
            {isEditMode ? t("comment.editCommentInPost") : t("comment.createNewComment")}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit, createOnErrorHandler)}>
          ...
          <DialogFooter>
            ...
            <Button disabled={disabled} type="submit" className="gradient-gaming">
